...
with tab1:
    ...
    if uploaded_file is not None:
        ...
        with st.spinner("Processing..."):
            try:
                response = requests.post("http://backend:8000/", files=files)
        ...
        st.image(img, caption="Anonymize image")
...
with tab2:
    ...
    latency = get_metric(
        'histogram_quantile(0.95, rate(http_request_duration_seconds_bucket{job="backend", handler="/"}[1m]))'
    )
    ...
    resp = requests.get(
        ...
        params={
            "query": 'rate(http_requests_total{job="backend", handler="/"}[1m])',
